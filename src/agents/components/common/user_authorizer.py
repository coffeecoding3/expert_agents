...
from src.database.services import agent_service, membership_service, user_service
from src.memory.memory_manager import memory_manager
from src.schemas.user_schemas import UserInfo, user_info_to_dict
from src.utils.db_utils import get_db_session

logger = logging.getLogger("user_authorizer")
...
class UserAuthorizer:
    ...
    def _create_user_info_from_db_result(
        self, user_result: Dict[str, Any], user_id: str, agent_code: str
    ) -> UserInfo:
        ...
        authority_code = f"lge_{agent_code.lower()}"
    ...
    def get_user_info(
        self, user_id: str, agent_code: str = "caia"
    ) -> Optional[UserInfo]:
        """사용자 정보 조회 (엘지니 SSO 방식)"""
        try:
            # 먼저 메인 DB에서 사용자 정보 조회
            user_info = self._get_user_from_main_db(user_id, agent_code)
            if user_info:
                return user_info

            # 메인 DB에 없으면 IAM DB에서 조회하여 새로 생성
            return self._create_user_from_iam_db(user_id, agent_code)

        except Exception as e:
            logger.error(f"사용자 정보 조회 실패 (user_id: {user_id}): {e}")
            return None

    def _get_user_from_main_db(
        self, user_id: str, agent_code: str
    ) -> Optional[UserInfo]:
        """메인 DB에서 사용자 정보 조회"""
        ...
        try:
            with self._get_db_connection("main") as cursor:
                ...
                if user_result:
                    return self._create_user_info_from_db_result(
                        user_result, user_id, agent_code
                    )
                return None
    ...
    def _create_user_from_iam_db(
        ...
        try:
            with self._get_db_connection("iam") as cursor:
                    ...
                    user_authorities=[
                        {
                            ...
                            # LGenie DB 스키마 호환성을 위한 필드 (실제 권한 검증에는 authority_code 사용)
                            "message_filter": "LGE",
                ...
                # 로컬 DB에 사용자 저장
                self._save_user_to_main_db(user_info, agent_code)
                return user_info
    ...
    def _save_user_to_main_db(self, user_info: UserInfo, agent_code: str) -> int:
        ...
        try:
            with get_db_session() as db:
                ...
                # 에이전트 ID 조회
                agent = agent_service.get_by_code(db, agent_code.upper())
                if not agent:
                    logger.warning(
                        f"[DATABASE] 에이전트를 찾을 수 없습니다: {agent_code}"
                    )
                    return user_db_id

                # 에이전트 멤버십 생성/업데이트
                membership_service.create_or_update_membership(
                    db, user_db_id, agent.id, UserRole.USER.value, True, None
                )
    ...
    def _get_user_authorities(self, user_id: str, cursor) -> List[Dict[str, Any]]:
        ...
        Note: message_filter, chat_filter는 LGenie DB 스키마와의 호환성을 위해 포함됨.
        실제 권한 검증에는 authority_code가 사용됨.
        """
        try:
            authorities_query = """
            SELECT 
                ...
                'COMPANY_TYPE' as authority_type, 'LGE' as message_filter, CONCAT(UPPER(a.code), '_AGENT') as chat_filter
    ...
    def check_agent_authority(self, user_id: str, agent_code: str) -> bool:
        """에이전트 권한 확인"""
        try:
            user_info = self.get_user_info(user_id, agent_code)
            if not user_info:
                return False
            # membership 테이블에서 에이전트 멤버십 확인
            if self._check_membership_authority(user_id, agent_code):
                return True
            else:
                return False
            ...
            logger.error(
                f"[DATABASE] {agent_code.upper()} 권한 확인 실패 (user_id: {user_id}): {e}"
            )
            return False

    def _check_membership_authority(self, user_id: str, agent_code: str) -> bool:
        """membership 테이블에서 에이전트 멤버십 확인"""
        try:
            with get_db_session() as db:
                ...
                if not agent:
                    logger.warning(
                        f"[MEMBERSHIP] {agent_code.upper()} 에이전트를 찾을 수 없습니다."
                    )
                    return False
                ...
                result = db.execute(
                    membership_query, {"user_id": user_id, "agent_id": agent_id}
                )
                membership_row = result.fetchone()

                if not membership_row:
                    logger.debug(
                        f"[MEMBERSHIP] 사용자 {user_id}의 {agent_code.upper()} 멤버십이 없습니다."
                    )
                    return False
                ...
                if not enabled:
                    logger.error(
                        f"[MEMBERSHIP] 사용자 {user_id}의 {agent_code.upper()} 멤버십이 비활성화되어 있습니다."
                    )
                    return False
                ...
                if expires_at:
                    ...
                    try:
                        ...
                        if expires_date < datetime.now():
                            logger.error(
                                f"[MEMBERSHIP] 사용자 {user_id}의 {agent_code.upper()} 멤버십이 만료되었습니다."
                            )
                            return False
                    except Exception as e:
                ...
                logger.info(
                    f"[MEMBERSHIP] 사용자 {user_id}의 {agent_code.upper()} 멤버십이 활성화되어 있습니다."
                )
                return True

        except Exception as e:
            logger.error(
                f"[MEMBERSHIP] 멤버십 확인 실패 (user_id: {user_id}, agent_code: {agent_code}): {e}"
            )
            return False

    def _ensure_special_user_memory(
        self, user_id: str, user_info: UserInfo, agent_code: str
    ) -> None:
        ...
        try:
            ...
            # 에이전트 ID 가져오기
            agent_id = memory_manager.get_agent_id_by_code(agent_code.lower())
            if not agent_id:
                logger.error(f"{agent_code.upper()} 에이전트 ID를 찾을 수 없습니다.")
                return
    ...
    def close(self):
        ...
        try:
            ...
            logger.debug("User Authorizer가 종료되었습니다.")
        except Exception as e:
            logger.error(f"User Authorizer 종료 중 오류: {e}")
...
def get_authorizer() -> UserAuthorizer:
    """싱글톤 인스턴스 반환"""
    global _authorizer_instance
    if _authorizer_instance is None:
        _authorizer_instance = UserAuthorizer()
...
if __name__ == "__main__":
    ...
    # 테스트용: agent_code를 명령줄 인자로 받거나 기본값 사용
    agent_code = sys.argv[1] if len(sys.argv) > 1 else "caia"
    user_id = sys.argv[2] if len(sys.argv) > 2 else "test_user"

    authorizer = get_authorizer()
    user_info = authorizer.get_user_info(user_id=user_id, agent_code=agent_code)
    print(f"User info for {user_id} (agent: {agent_code}): {user_info}")

    # 권한 확인 예시
    has_authority = authorizer.check_agent_authority(user_id, agent_code)
    print(f"Has {agent_code.upper()} authority: {has_authority}")
